...
public class BSTreeMap<K extends Comparable<K>, V> implements MyMap<K, V> {
    ...
    protected Node<K, V> createBST(Pair<K, V>[] pairs, int low, int high) 
    {
        ...
        else
        {
            ...
            Node<K, V> leftNode = createBST(pairs, low, mid - 1);
            node.setLeft(leftNode);
            if (leftNode != null) {
                leftNode.setParent(node);
            }
            Node<K, V> rightNode = createBST(pairs, mid + 1, high);
    ...
    public V remove(K key) 
    {
        ...
        while (current != null)
        {
            if (current.key.compareTo(key) == 0)
            {
                Node<K,V> rightNode = current.getRight();
                Node<K,V> leftNode = current.getLeft();
                ...
                else
                {
                    ...
                    if (rightNode.getParent() != current)
                    {
                        transplant(rightNode, rightNode.getRight());
                        rightNode.setRight(current.getRight());
                        rightNode.getRight().setParent(rightNode); 
                    }
                    transplant(current, rightNode);
                    rightNode.setLeft(current.getLeft());
